...
def summarize_multi_documents(
    ...
    """
    Summarize a collection of documents into a single unified summary.
    """
    start_time = time.time()
    ...
    for filename, text in documents:
        # Preprocess each document and get its summary
        res = summarize_text(
            text=text,
            max_length=max_length,
            min_length=min_length,
        ...
        individual_summaries.append(f"Summary of {filename}: {res['summary']}")
        total_original_words += res['original_word_count']
        
    # Combine the main points of all documents
    combined_context = "\n".join(individual_summaries)
    
    # final pass to reduce redundancy and generate a coherent final unified summary
    final_summary = model_service.summarize_chunk(
        combined_context,
        max_length=max_length,
        min_length=min_length,
    ...
    # Add final aggregated summary to history
